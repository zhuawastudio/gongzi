        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '薪资编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.xinziUuidNumber" placeholder="薪资编号" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '月份' : ''">
                         <el-input prefix-icon="el-icon-search" type="month" v-model="searchForm.xinziMonth" placeholder="月份" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xinzi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/gongzi/upload/xinziMuBan.xls"
                        >批量导入套账数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xinzi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xinziUploadSuccess"
                                :on-error="xinziUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xinzi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xinzi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xinziUuidNumber"
                                   header-align="center"
                                   label="薪资编号">
                        <template slot-scope="scope">
                            {{scope.row.xinziUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xinzi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xinzi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xinzi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'xinzi',//当前表表名,
                        sumColum : 'xinzi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xinzi_name',//分组字符串字段
                        // types : 'xinzi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.xinziUuidNumber!= '' && this.searchForm.xinziUuidNumber!= undefined) {
                    params['xinziUuidNumber'] = '%' + this.searchForm.xinziUuidNumber + '%'
                }
         
                if (this.searchForm.xinziMonth!= '' && this.searchForm.xinziMonth!= undefined) {
                    params['xinziMonth'] = '%' + this.searchForm.xinziMonth + '%'
                }
                                                        
                params['xinziDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            xinziUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xinzi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
